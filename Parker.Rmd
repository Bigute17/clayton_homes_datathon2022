...
test_b <- FULL_DATASET %>%
  ...
  summarise(mean_ZHVI = mean(ZHVI, na.rm = T)) %>%
  group_by(State) %>%
  summarize(improvement_rate_2015_to_2021= max(mean_ZHVI)/min(mean_ZHVI)) %>%
  arrange(desc(improvement_rate_2015_to_2021))

head_test_b <- head(test_b)
  

ggplot(head_test_b, aes(State, improvement_rate_2015_to_2021)) + geom_boxplot()
```
